...
class BatchGenerator:
    ...
    Returned batches have the following properties:
      data: numpy array holding batch data of shape
            (s, SHAPE_OF_DATASET_SAMPLES).
      label: numpy array holding batch labels of shape
            (s, SHAPE_OF_DATASET_LABELS).
      idx: numpy array with shape (s,) encoding the indices
           of each sample in the original dataset.
    '''

    def __init__(self, dataset: Dataset, num: int, shuffle: bool,
                 op: Op = None):
        '''
        Ctor.
        Dataset is the dataset to iterate over.
        num is the number of samples per batch. the number in the last batch
        ...
        Raises ValueError on invalid argument values, such as if
        num is > len(dataset).
        '''
        ...
        if num > len(dataset):
            raise ValueError(
                "Number of samples in batch must be smaller than total number")
